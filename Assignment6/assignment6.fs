...
let rec expand num den radix =
  let final = ((num*radix)/den)
  cons final (Seq.delay (fun () -> expand ((num*radix)-(final*den)) den radix)
...
let rec integrateSeries sigma = 
  cons (integrateTerm (first sigma)) (Seq.delay (fun () -> integrateSeries (rest sigma)))
(* Just 2 lines of code! *)
...
let rec sumSeries (sigma: seq<term>) (x: float) (n: int) : float =
  if n=0 then 0.0 else ((evalTerm (first sigma) x) + (sumSeries (rest sigma) x (n-1)))
(* Just 2 lines of code! *)
